...
//Bağlı listenin başını gösteren root her değiştiğinde baştan çalışarak sırayla index atar. Bağlı elemanlar eklenirken sıralı olduğu için 0 dan başlayıp teker teker arttırır.
void indexAyarla(node * r){
  int i = 0;
  while (r != NULL){
    r->index = i;
...
node * sil(node* r, int index){
  ...
  if(iter->next == NULL){
    printf("Silmek istediğiniz numara tabloda yok.\n");
    return r;
  }
  temp = iter->next;
  iter->next = temp->next;
...
node * degistir(node * r, int numara, int yeniNot) {
  ...
  /*
  Değiştir fonksiyonu değiştirmeyi istediğimiz öğrenciyi önce siler, sonrasında yeni not değeriyle ekler. Bu yüzden de eğer öğrenciyi silerken veya eklerken bazı durumlara bakmalıydım. Eğer en yüksek nota ve numara önceliğine sahip ilk öğrenciyi siliyorsam linked list'in başını gösteren root değişebilir dolayısıyla yeniRoot diye bir pointer tanımlayıp sil fonksiyonundan dönen pointeri burada tutuyorum. Daha sonrasında ekle fonksiyonunu çağırırken bu yeni pointeri kullanıyorum ve eklerken eğer değişen değer başa eklenecek bir değerse root değişecek diye de bu ekle fonksiyonunun return ettiği sonucunu return ediyorum.
  */
